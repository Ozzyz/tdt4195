...
unsigned int createCircleVAO(unsigned int slices, unsigned int layers, float* color) {
	...
	const unsigned int verticeCount = triangleCount * VERTICES_PER_TRIANGLE;
	float* vertices = new float[verticeCount * COMPONENTS_PER_VERTEX];
	unsigned int* indices = new unsigned int[verticeCount];
	...
	unsigned int vao_id = setupVAO(vertices, verticeCount*COMPONENTS_PER_VERTEX, indices, verticeCount, colors);
